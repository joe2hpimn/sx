...
and \SX was installed into \verb+/opt/sx+. Also in some cases (eg. the path to
...
nodes. Additionally, we create an alias \textbf{@cluster}, which later
can be used instead of\\ \textbf{sx://admin@mycluster}.
\small
\begin{lstlisting}
...
groups of users and managed with the \verb+sxvol+ tool. Below we create
a basic volume of size 50G owned by admin and fully replicated on two nodes.
We're also making use of the @cluster alias, which stands for\\ sx://admin@mycluster.
\small
\begin{lstlisting}
...
\end{lstlisting}
\LARGE
Use \verb+sxcp -r+ to recursively upload directories to \SX. See the man pages
(eg. \verb+man sxcp+) for examples and usage details.
