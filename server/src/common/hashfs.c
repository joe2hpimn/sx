...
struct _sx_hashfs_t {
    ...
    sqlite3_stmt *qb_gc_list_reservation_revs[SIZES][HASHDBS];
    sqlite3_stmt *qb_gc_find_expired_reservation[SIZES][HASHDBS];
    sqlite3_stmt *qb_gc_revision_blocks[SIZES][HASHDBS];
    sqlite3_stmt *qb_gc_revision_ops[SIZES][HASHDBS];
    sqlite3_stmt *qb_gc_reservation[SIZES][HASHDBS];
...
static void close_all_dbs(sx_hashfs_t *h) {
	...
	for(i=0; i<HASHDBS; i++) {
	    ...
	    sqlite3_finalize(h->qb_bumpalloc[j][i]);
            ...
            sqlite3_finalize(h->qb_gc_find_inactive_reservation[j][i]);
            sqlite3_finalize(h->qb_gc_list_reservation_revs[j][i]);
            sqlite3_finalize(h->qb_gc_find_expired_reservation[j][i]);
            sqlite3_finalize(h->qb_gc_revision_blocks[j][i]);
            sqlite3_finalize(h->qb_gc_revision_ops[j][i]);
            sqlite3_finalize(h->qb_gc_reservation[j][i]);
...
sx_hashfs_t *sx_hashfs_open(const char *dir, sxc_client_t *sx) {
	...
	goto open_hashfs_fail;
    if(qprep(h->tempdb, &h->qt_tokendata, "SELECT tid, size, volume_id, name, content, t || ':' || token AS revision FROM tmpfiles WHERE token = :token AND flushed = 0"))
	goto open_hashfs_fail;
	...
	for(i=0; i<HASHDBS; i++) {
		...
		goto open_hashfs_fail;
	    if(qprep(h->datadb[j][i], &h->qb_gc_block[j][i], "DELETE FROM blocks WHERE id = :blockid"))
		goto open_hashfs_fail;
		...
		goto open_hashfs_fail;
            if(qprep(h->datadb[j][i], &h->qb_revmap_op[j][i], "INSERT OR IGNORE INTO revision_ops(revision_id, op, age) VALUES(:revision_id, :op, :age)"))
                goto open_hashfs_fail;
            /* OR IGNORE to avoid subjournal */
            if(qprep(h->datadb[j][i], &h->qb_revmap_create[j][i], "INSERT OR IGNORE INTO revision_blocks(revision_id, blocks_hash, age, replica, global_vol_id) VALUES(:revision_id, :hash, :age, :replica, :global_vol_id)"))
                goto open_hashfs_fail;
	    ...
	    */
            if(qprep(h->datadb[j][i], &h->qb_gc_find_block[j][i], "SELECT id, blockno, hash FROM blocks WHERE hash IN ( SELECT blocks_hash FROM revision_blocks WHERE blocks_hash IN ( SELECT blocks_hash FROM revision_blocks WHERE revision_id=:revision_id AND blocks_hash > :prev ORDER BY blocks_hash) GROUP BY blocks_hash HAVING COUNT(*)=1 ORDER BY blocks_hash) LIMIT 1"))
                goto open_hashfs_fail;
            ...
            if(qprep(h->datadb[j][i], &h->qb_deleteold[j][i], "DELETE FROM revision_blocks WHERE blocks_hash=:hash AND age < :current_age"))
                goto open_hashfs_fail;
            if(qprep(h->datadb[j][i], &h->qb_gc_find_inactive_reservation[j][i], "SELECT reservations_id FROM reservations NATURAL INNER JOIN revision_blocks INNER JOIN blocks ON blocks.hash = blocks_hash WHERE reservations_id > :prev GROUP BY reservations_id HAVING MAX(created_at) < :expires ORDER BY reservations_id LIMIT 1"))
               goto open_hashfs_fail;
            if(qprep(h->datadb[j][i], &h->qb_gc_list_reservation_revs[j][i], "SELECT revision_id FROM reservations WHERE reservations_id = :reservation_id AND revision_id > :prev ORDER BY revision_id LIMIT 1"))
               goto open_hashfs_fail;
            /* The following query intentionally refuses to index by a revision ID, because indexing by the tll is much better,
             * the indexed values are much less frequent */
            if(qprep(h->datadb[j][i], &h->qb_gc_find_expired_reservation[j][i], "SELECT MIN(+revision_id) FROM reservations WHERE ttl < :ttl AND revision_id > :prev"))
               goto open_hashfs_fail;
            if(qprep(h->datadb[j][i], &h->qb_gc_revision_blocks[j][i], "DELETE FROM revision_blocks WHERE revision_id=:revision_id"))
                goto open_hashfs_fail;
            if(qprep(h->datadb[j][i], &h->qb_gc_revision_ops[j][i], "DELETE FROM revision_ops WHERE revision_id=:revision_id"))
                goto open_hashfs_fail;
            if(qprep(h->datadb[j][i], &h->qb_gc_reservation[j][i], "DELETE FROM reservations WHERE revision_id=:revision_id"))
                goto open_hashfs_fail;
...
static rc_ty hashop_create_revmap(sx_hashfs_t *h, const sx_hash_t *global_vol_id, const sx_hash_t *revision_id, unsigned int hs, const sx_hash_t *hash, unsigned replica);
        ...
        do {
            ...
            while ((ret = qstep(qsel)) == SQLITE_ROW && (k++ < BULK_MAX_BATCH)) {
                ...
                for (j=0;j<blocks;j++) {
                    ...
                    if (hashop_create_revmap(h, &vol->global_id, &revision_id, hs, hash, replica))
                        break;
                }
                if (j != blocks)
                    break;
...
/*
 * Perform an operation on revision_ops and reservations tables.
 *
 * When op is 1 we insert a 1 op into revision_ops table from all the hash databases
 * and we delete all reservations which contain the provided revision_id from reservations table
 * from all the hash databases.
 *
 * When op is -1 we only insert a -1 op into revision ops table from all the hash databases. Normally
 * a file deletion should occur after a file is uploaded, therefore no reservations should exist at this point.
 * However, if the file is not yet uploaded and we issue its deletion (probably due to a race condition), then
 * it is unlikely that we have a reservation there. When a positive op comes, the reservation should be eventually
 * deleted. In the worst case the reservation would expire.
 *
 * Note: There is no transaction handled, making a transaction is a responsibility of the caller,
 *       see sx_hashfs_revision_op_{begin,commit,rollback} functions.
 */
rc_ty sx_hashfs_revision_op(sx_hashfs_t *h, unsigned blocksize, const sx_hash_t *revision_id, int op)
{
    ...
    for(ndb = 0; ndb < HASHDBS; ndb++) {
        sqlite3_stmt *q = h->qb_revmap_op[hs][ndb];
        ...
        if(op > 0) {
            ...
            /* Delete all the reservations sharing this revision ID */
            sqlite3_reset(qdel);
            if(qbind_blob(qdel, ":revision_id", revision_id->b, sizeof(revision_id->b)) || qstep_noret(qdel)) {
                sqlite3_reset(qdel);
                return FAIL_EINTERNAL;
            }
            sqlite3_reset(qdel);
...
/* Check if a hash is stored in database */
static rc_ty hashop_check_presence(sx_hashfs_t *h, unsigned hs, const sx_hash_t *hash)
{
    rc_ty ret;
    unsigned ndb;
    ndb = gethashdb(hash);
...
/*
 * Populate a reservation for a revision ID.
 *
 * Reservations are created in all the block databases which match the block size. A reservation consists of
 * a reservation ID which is calculated at tempfile creation time and a file revision ID. Reservations of different
 * revision IDs but for the same destination file share the same reservations ID.
 *
 * A reservation has an expiration time which lets GC to clean up those reservations which were introduced
 * by unfinished uploads (e.g. initerrupted uploads).
 *
 * There is no transaction created for the hash databases. It is the caller responsibility to perform a transaction
 * among the databases. See sx_hashfs_revision_op_{begin,commit,rollback} for reference.
 */
rc_ty sx_hashfs_reserve_revision_id(sx_hashfs_t *h, const sx_hash_t *reserve_id, const sx_hash_t *revision_id, unsigned int blocksize, uint64_t op_expires_at) {
    ...
    DEBUGHASH("Reserving the revision ID", revision_id);
    for(ndb = 0; ndb < HASHDBS; ndb++) {
        ...
        /*
         * Creating 0-op entries is required for the GC to properly handle reservations.
         *
         * The situation which requires this is as the following:
         * - we upload 2 files which consist of a single block, shared by both files.
         * - we don't flush any file.
         * - one of the files' reservation expires. GC has to delete one of 2 reservations.
         * - The GC picks the expired reservation and for each block which belongs to the expired
         *   reservation's revision ID it checks if the block is shared by any of the existing revision ID's.
         * - Because the 0-op entries exist for both revision IDs sharing the same block, the block cannot be deleted.
         *   If the 0-op entry did not exist we'd drop the block, because the particular revision ID would no longer
         *   be reserved and the revision ID would not share its block with any other revision ID.
         */
        if (qbind_blob(h->qb_revmap_op[hs][ndb], ":revision_id", revision_id, sizeof(*revision_id)) ||
            qbind_int(h->qb_revmap_op[hs][ndb], ":op", 0) ||
            qbind_int64(h->qb_revmap_op[hs][ndb], ":age", age) ||
            qstep_noret(h->qb_revmap_op[hs][ndb])) {
            sqlite3_reset(h->qb_revmap_op[hs][ndb]);
            return FAIL_EINTERNAL;
        }
        sqlite3_reset(h->qb_revmap_op[hs][ndb]);
    }
...
/*
 * Create a revmap entry.
 *
 * A revmap is a connection between blocks and revision ID stored in revision_blocks table.
 */
static rc_ty hashop_create_revmap(sx_hashfs_t *h, const sx_hash_t *global_vol_id, const sx_hash_t *revision_id, unsigned int hs, const sx_hash_t *hash, unsigned replica)
{
    unsigned ndb;
     ...
     *
     * The situation when the volume ID is not available is only if some files
     * were stored on an old cluster, got deleted but GC didn't clean them up before a remote upgrade (2.0 -> 2.1)
     * started. The remote update would be unable to find a proper voulume ID for them.
     */
    if(!global_vol_id && qbind_null(q, ":global_vol_id"))
        return FAIL_EINTERNAL;
    else if(global_vol_id && qbind_blob(q, ":global_vol_id", global_vol_id, sizeof(*global_vol_id)))
        return FAIL_EINTERNAL;

    if(qbind_blob(q, ":hash", hash, sizeof(*hash)) ||
       qbind_int(q, ":replica", replica) ||
       qbind_int64(q, ":age", age) ||
       qbind_blob(q, ":revision_id", revision_id, sizeof(*revision_id)) ||
       qstep_noret(q)) {
        msg_set_reason("Failed to create a revmap");
        sqlite3_reset(q);
        return FAIL_EINTERNAL;
    }
...
rc_ty sx_hashfs_hashop_perform(sx_hashfs_t *h, unsigned int block_size, unsigned replica_count, enum sxi_hashop_kind kind, const sx_hash_t *hash, const sx_hash_t *global_vol_id, const sx_hash_t *reserve_id, const sx_hash_t *revision_id, uint64_t op_expires_at, int *present)
{
    ...
    if (kind == HASHOP_SKIP) {
        /* When HASHOP_SKIP is used we are reuploading the same file revision which should skip all the operations.
         * Since we don't want to perform a presence check by using HASHOP_SKIP, therefore we assume everything is in place.
         * Otherwise the client would be told to upload the blocks. */
        if(present)
            *present = 1;
        return OK;
    }
    ...
    if(kind != HASHOP_CHECK && kind != HASHOP_RESERVE) {
        msg_set_reason("Invalid hashop kind");
        return EINVAL;
    }

    if(kind == HASHOP_RESERVE) {
        rc = hashop_create_revmap(h, global_vol_id, revision_id, hs, hash, replica_count);
        if(rc != OK)
            goto sx_hashfs_hashop_perform_err;
    }

    /* We always check hash presence after hashop_create_revmap() call */
    rc = hashop_check_presence(h, hs, hash);
sx_hashfs_hashop_perform_err:
...
/*
 * Create a revmap and mark it as used.
 *
 * A revmap is a connection between blocks and revision ID which is stored in revision_blocks table.
 * Revision ID usage is stored in revision_ops table. This function creates the connection and mark the revision
 * as used, so it won't be gced.
 *
 * Note: This function writes to all the block databases which match the blocksize provided and does not use a transaction.
 *       If you want to make the insertion atomic you should you a transaction (via sx_hashfs_revision_op_{begin,commit,rollback} calls).
 */
rc_ty sx_hashfs_hashop_use_revmap(sx_hashfs_t *h, const sx_hash_t *hash, const sx_hash_t *global_vol_id, const sx_hash_t *revision_id, unsigned int bs, unsigned replica)
{
    unsigned int hs;
    rc_ty rc;

    if(!h || !hash || !revision_id) {
        NULLARG();
        return EINVAL;
    }
    ...
    /* This call accesses all the hash databases matching the block size */
    rc = sx_hashfs_revision_op(h, bs, revision_id, 1);
    if(rc != OK) {
        WARN("Failed to mark revision as used");
        return rc;
    }

    rc = hashop_create_revmap(h, global_vol_id, revision_id, hs, hash, replica);
    if (rc == OK)
        rc = hashop_check_presence(h, hs, hash);
    return rc;
}
...
static rc_ty are_blocks_available(sx_hashfs_t *h, sx_hash_t *hashes,
	...
	int present = 0; /* Will be set to 1 if the hash is stored on the local node */
        rc_ty rc;

	*current = check_item+1;
        if (sxi_hashop_batch_flush(hdck))
            WARN("Failed to query hash: %s", sxc_geterrmsg(h->sx));
        rc = sx_hashfs_hashop_perform(h, bsz[hash_size], hdck->replica, hdck->kind, hash, &hdck->global_vol_id, &hdck->reserve_id, &hdck->revision_id, hdck->op_expires_at, &present);
        if(rc != OK) {
            WARN("hashop_perform/finish failed: %s", rc2str(rc));
            return rc;
        }
	if (hdck->cb) {
	    int code = present ? 200 : 404;
...
rc_ty sx_hashfs_putfile_commitjob(sx_hashfs_t *h, const uint8_t *user, sx_uid_t user_id, const char *token, job_t *job_id, int fastreturn) {
    ...
    rev = (const char *)sqlite3_column_text(h->qt_tokendata, 5);
    ...
    /* Calculate a revision ID, it should be used for running revid bump job */
    if(sx_unique_fileid(h->sx, rev, &revop_ctx.revision_id)) {
        WARN("Failed to compute revision ID");
        goto putfile_commitjob_err;
    }
    revop_ctx.blocksize = blocksize;
    revop_ctx.op = 1; /* Its a revid bump job */
    revop_ctx.lock = token; /* We use the same lockname as the other flush jobs use */
    ...
    *job_id = JOB_NOPARENT;
    ...
    /* Creates a new JOBTYPE_BLOCKS_REVISION job which is handled by the _2pc calls,
     * stick to this design here. All the necessary details are set in the revop_ctx structure. */
    ret = sx_hashfs_job_new_2pc_notrigger(h, &revision_spec, &revop_ctx, user_id, job_id);
    if(ret != OK) {
        INFO("job_new (rev bump) returned: %s", rc2str(ret));
        goto putfile_commitjob_err;
    }

    if(vol->effective_replica == 1 || !fastreturn) {
	/* The flush completes when all replicas are complete */
	ret = sx_hashfs_job_new_notrigger(h, *job_id, user_id, job_id, JOBTYPE_REPLICATE_BLOCKS, sx_hashfs_job_file_timeout(h, ndests, expected_size), token, &tmpfile_id, sizeof(tmpfile_id), singlenode);
	if(ret) {
	...
	ret = sx_hashfs_job_new_notrigger(h, *job_id, user_id, job_id, JOBTYPE_REPLICATE_BLOCKS_FG, sx_hashfs_job_file_timeout(h, ndests, expected_size), token, &tmpfile_id, sizeof(tmpfile_id), singlenode);
	if(ret) {
	...
	for(i=1; i<=tbd->replica_count; i++) {
		...
		continue;
            ...
            sxi_hashop_begin(&h->hc, h->sx_clust, tmp_getmissing_cb,
                             HASHOP_CHECK, tbd->replica_count, &volume->global_id, &reserve_id, &tbd->revision_id, tbd, 0);
...
/* Delete a hash */
static rc_ty gc_block(sx_hashfs_t *h, unsigned int hs, unsigned int hdb, const sx_hash_t *hash, int64_t id, int64_t blockno)
{
    sqlite3_stmt *q_setfree;
    sqlite3_stmt *q_gc;

    if(!h || hs >= SIZES || hdb >= HASHDBS || !hash) {
        WARN("Invalid argument");
        return EINVAL;
    }
    q_setfree = h->qb_setfree[hs][hdb];
    q_gc = h->qb_gc_block[hs][hdb];

    if (sx_hashfs_blkrb_can_gc(h, hash, bsz[hs]) != OK) {
    ...
    DEBUG("freeing blockno %ld, @%d/%d/%ld", blockno, hs, hdb, blockno * bsz[hs]);
...
static rc_ty gc_revision_blocks(sx_hashfs_t *h, int *terminate, unsigned int hs, unsigned int hdb, const sx_hash_t *revid, uint64_t *gced_blocks, uint64_t *skipped_blocks) {
    ...
    do {
        ...
        if(qbind_blob(q, ":revision_id", revid->b, sizeof(revid->b)) ||
           qbind_blob(q, ":prev", hash.b, sizeof(hash.b))) {
        ...
        /* Drop the hash */
        s = gc_block(h, hs, hdb, &hash, id, blockno);
        if(s == OK)
            gced++;
        else if(s == EAGAIN) {
 ...
 *
 * The delete_reservation flag is set when we want to delete an entry from reservations table. When we delete a revision
 * which is fully unbumped by a block manager we are not deleting a reservation for it, therefore it is not necessary
 * to call a query which would eventually be a no-op.
 */
static rc_ty gc_revision(sx_hashfs_t *h, int *terminate, unsigned int hs, unsigned int hdb, int delete_reservation, const sx_hash_t *revid, uint64_t *gced_blocks, uint64_t *skipped_blocks) {
    ...
    /*
     * When we iterate over expired or inactive reservations we want to 
     * delete a reservations table entry corresponding to the revision provided.
     * When it is needed to delete the reservation, the delete_reservation flag should be set.
     *
     * We run this query after deleting all revision entries (blocks, ops etc.),
     * because we don't want to lose a reference to the revision in case
     * deleting the revision failed after the entry from reservations table was dropped.
     */
    if(delete_reservation && qstep_noret(qreservation))
        goto gc_revision_err;
...
static rc_ty unused_revisions_iterate(sx_hashfs_t *h, int *terminate, unsigned int hs, unsigned int hdb, sx_hash_t *last_revid, const void *context, uint64_t *processed_revids, uint64_t *gced_revids, uint64_t *gced_blocks, uint64_t *skipped_blocks) {
    ...
    do {
        ...
        gced = 0;
        skipped = 0;
        s = gc_revision(h, terminate, hs, hdb, 0, &revid, &gced, &skipped);
        if(s == OK || s == EAGAIN) {
...
static rc_ty inactive_revision_ids_iterate(sx_hashfs_t *h, int *terminate, unsigned int hs, unsigned int hdb, const sx_hash_t *reservation_id, uint64_t *processed_revids, uint64_t *gced_revids, uint64_t *gced_blocks, uint64_t *skipped_blocks) {
    ...
    q = h->qb_gc_list_reservation_revs[hs][hdb];
    sqlite3_reset(q);

    /* Memset to prepare a start point for the iteration */
    memset(revid.b, 0, sizeof(revid.b));
    do {
        int r;
        rc_ty s;

        if(qbind_blob(q, ":reservation_id", reservation_id->b, sizeof(reservation_id->b)) ||
           qbind_blob(q, ":prev", revid.b, sizeof(revid.b))) {
            WARN("Failed to prepare selection query");
            goto inactive_revision_ids_iterate_err;
        }
        ...
        /* The query can be reset now */
        sqlite3_reset(q);

        /* GC the revision */
        gced = 0;
        skipped = 0;
        s = gc_revision(h, terminate, hs, hdb, 1, &revid, &gced, &skipped);
        if(s == OK || s == EAGAIN) {
            /* When EAGAIN is returned it means we intentionnaly do *not* delete
             * the revision, but some blocks might be dropped. The rationale is
             * a running rebalance might lock some blocks to not be gc'ed. In such a case
    ...
    ret = OK;
inactive_revision_ids_iterate_err:
    sqlite3_reset(q);
    return ret;
}
...
static rc_ty inactive_reservations_iterate(sx_hashfs_t *h, int *terminate, unsigned int hs, unsigned int hdb, sx_hash_t *last_reservation, const void *context, uint64_t *processed_revids, uint64_t *gced_revids, uint64_t *gced_blocks, uint64_t *skipped_blocks) {
    ...
    const int64_t *expires = (const int64_t *)context;
    const uint8_t *ptr;
    double elapsed = 0.0;
    ...
    q = h->qb_gc_find_inactive_reservation[hs][hdb];
    sqlite3_reset(q);
    ...
    do {
        ...
        if(qbind_int64(q, ":expires", *expires) ||
           qbind_blob(q, ":prev", last_reservation->b, sizeof(last_reservation->b))) {
            WARN("Failed to prepare selection query");
        ...
        /* Take the revision do delete */
        ptr = sqlite3_column_blob(q, 0);
        if(!ptr || sqlite3_column_bytes(q, 0) != sizeof(reservation_id.b)) {
            WARN("Invalid revision ID found");
            goto inactive_reservations_iterate_err;
        }

        /* Store the revision */
        memcpy(reservation_id.b, ptr, sizeof(reservation_id.b));
        ...
        if(s == ITER_NO_MORE) {
            /* The iteration over revision ID's bound to this reservation is finished now.
             * We can move the iteration further by bumping the iteration threshold. */
            memcpy(last_reservation->b, reservation_id.b, sizeof(last_reservation->b));
        } else {
            /* Reruning OK means we did not finish the iteration due to a timeout, we should break the loop now */
            break;
        }
    ...
    ret = OK;
inactive_reservations_iterate_err:
    sqlite3_reset(q);
    return ret;
}
...
static rc_ty expired_reservations_iterate(sx_hashfs_t *h, int *terminate, unsigned int hs, unsigned int hdb, sx_hash_t *last_revid, const void *context, uint64_t *processed_revids, uint64_t *gced_revids, uint64_t *gced_blocks, uint64_t *skipped_blocks) {
    ...
    sqlite3_stmt *q;
    sx_hash_t revid;
    const int64_t *ttl = (const int64_t *)context;
    const uint8_t *ptr;
    uint64_t gced = 0, skipped = 0; /* A number of blocks GCed for each revision */
    double elapsed = 0.0;
    ...
    q = h->qb_gc_find_expired_reservation[hs][hdb];
    sqlite3_reset(q);

    do {
        rc_ty s;

        if(qbind_int64(q, ":ttl", *ttl) ||
           qbind_blob(q, ":prev", last_revid->b, sizeof(last_revid->b))) {
            WARN("Failed to prepare selection query");
            goto expired_reservations_iterate_err;
        }

        /* This query returns a MIN(revision_id), it always returns. */
        if(qstep_ret(q)) {
            WARN("Failed to find revisions to gc");
            goto expired_reservations_iterate_err;
        }
    ...
    ret = OK;
expired_reservations_iterate_err:
    sqlite3_reset(q);
    return ret;
}
    ...
    for(j = 0; j < SIZES; j++) {
        for(i = 0; i < HASHDBS; i++) {
            ...
            do {
                ...
                if(skipped_blocks)
                    (*skipped_blocks) += gced_blocks_now;
                ...
                /* Check the return code of the callback and stop if failed. */
...
rc_ty sx_hashfs_gc_periodic(sx_hashfs_t *h, int *terminate, int grace_period)
{
    ...
    if (qbind_int64(h->qt_gc_revisions, ":now", now) ||
        ...
        return FAIL_EINTERNAL;
    gettimeofday(&end, NULL);
    INFO("GCed %d tokens in %.2lfs", sqlite3_changes(h->tempdb->handle), sxi_timediff(&end, &start));
    ...
    gettimeofday(&end, NULL);
    INFO("GCed %llu tombstones in %.2lfs", (unsigned long long)tombstones_dropped, sxi_timediff(&end, &start));
    ...
    if (qstep_noret(h->qe_gc))
        return FAIL_EINTERNAL;
    gettimeofday(&end, NULL);
    INFO("GCed %d jobs in %.2lfs", sqlite3_changes(h->eventdb->handle), sxi_timediff(&end, &start));
 ...
 * Returns oter code in case of error.
 * This function is a part of the gc's slow check.
 *
 * === This function must be called inside a transaction. ===
 *
 * Args:
 *      sizedb, hashdb: hash database indices
 *      last: a rowid of the hash to start from, works as an output argument to serve for the next iteration attempt.
 *      blocks_gced: an output parameter where the number of gced blocks is stored.
 */
static rc_ty unused_blocks_iterate(sx_hashfs_t *h, int *terminate, unsigned int sizedb, unsigned int hashdb, int64_t *last, uint64_t *gced) {
    rc_ty ret = FAIL_EINTERNAL;
    sqlite3_stmt *q;
    int r;
    uint64_t rows = 0;

    if(!h || !terminate || !last || !gced) {
        NULLARG();
        return EINVAL;
    }
    ...
    q = h->qb_gc_find_unused_block[sizedb][hashdb];
    ...
    while((r = qstep(q)) == SQLITE_ROW && !*terminate) {
         ...
         */
        r = gc_block(h, sizedb, hashdb, &hash, id, blockno);
        if (r == OK)
            (*gced)++;
        else if (r != EAGAIN)
            goto unused_blocks_iterate_err;
    ...
    /* When termination is requested we don't want the run to be treated as a failure,
     * therefore we return OK here. */
    if(*terminate || rows >= GC_MAX_ROWS)
        ret = OK;
    else
        ret = ITER_NO_MORE;
unused_blocks_iterate_err:
    DEBUG("[%d/%d] %s: %llu/%llu", sizedb, hashdb, ret == OK ? "throttled" : (ret == ITER_NO_MORE ? "done" : "failure"),
        (unsigned long long)*gced, (unsigned long long)rows);
    sqlite3_reset(q);
    return ret;
}
...
rc_ty sx_hashfs_gc_slow(sx_hashfs_t *h, int *terminate)
{
    ...
    for (j=0;j<SIZES && !ret && !*terminate ;j++) {
        for (i=0;i<HASHDBS && !ret && !*terminate;i++) {
            ...
            do {
                ...
                /* Iterate over blocks, check if they're used and drop when needed */
                r = unused_blocks_iterate(h, terminate, j, i, &last, &gced_now);
                gc_blocks += gced_now;

                /* Commit the changes only if some blocks were touched
                 * (gced_now is not 0) and there was no failure.
                 * In case termination was requested we roll the changes back.
                 */
                if(gced_now && (r == OK || r == ITER_NO_MORE) && !*terminate) {
                    if (qcommit(h->datadb[j][i])) {
                        ret = -1;
                        break;
                    }
                ...
                /* Check the return code of the unused_blocks_iterate() and stop if failed. */
                if(r != OK && r != ITER_NO_MORE) {
                    ret = -1;
                    break;
                }
    ...
    gettimeofday(&end, NULL);
    INFO("GCed %lld unused hashes in %.2lfs", (long long)gc_blocks, sxi_timediff(&end, &start));
...
static rc_ty volume_chown_privs(sx_hashfs_t *h, const sx_hashfs_volume_t *vol, int64_t newid) {
    rc_ty ret = FAIL_EINTERNAL;

    if(!vol || newid == vol->owner) {
        WARN("Failed to change volume ownership: incorrect argument");
        msg_set_reason("Incorrect argument");
        return EINVAL;
    }

    /* Grant privs for new volume owner */
...
static rc_ty volume_mod_common(sx_hashfs_t *h, const sx_hashfs_volume_t *vol, const char *newname, int64_t size, int64_t newowner, int max_revisions) {
    rc_ty ret = FAIL_EINTERNAL;
    ...
    if(qbind_text(h->q_modvol, ":name", newname)
       || qbind_int64(h->q_modvol, ":size", size)
       || qbind_int64(h->q_modvol, ":owner", newowner)
       || qbind_int(h->q_modvol, ":revs", max_revisions)
...
rc_ty sx_hashfs_volume_mod(sx_hashfs_t *h, const sx_hashfs_volume_t *vol, const char *newname, const char *newowner, int64_t newsize, int max_revs, int modify_meta) {
    ...
    if(max_revs != -1) {
        if((unsigned int)max_revs > SXLIMIT_MAX_REVISIONS && (unsigned int)max_revs < SXLIMIT_MIN_REVISIONS) {
            WARN("Invalid revisions limit given");
            ret = s;
            goto sx_hashfs_volume_mod_err;
        }
    } else
        max_revs = vol->revisions;

    /*
     * Modify volume, changing owner requires also privs modification, see below.
     * When a value is not to be changed, it must has the same value as vol instance has.
     */
    if((s = volume_mod_common(h, vol, newname, newsize, newid, max_revs)) != OK) {
        ret = s;
        goto sx_hashfs_volume_mod_err;
    }

    /* In case we are chowning the volume, we need to move privs */
    if(newid != vol->owner && (s = volume_chown_privs(h, vol, newid)) != OK) {
        ret = s;
        goto sx_hashfs_volume_mod_err;
    }
