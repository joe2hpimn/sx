...
#define _BSD_SOURCE
#define VOLSIZE args.replica_arg*1024LL*1024LL*1024LL /* TODO: bigger precision */
...
void create_block(rnd_state_t *state, unsigned char *block, uint64_t block_size)
{
    ...
    for(i=0; i<block_size; i++)
        block[i] = rand_2cmres(state) & UCHAR_MAX;
} /* create_block */
...
int test_attribs(sxc_client_t *sx, sxc_cluster_t *cluster, char *local_dir_path, char *remote_dir_path) {
    ...
    printf("test_attribs: Started (experimental)\n");
    memset(files, 0, ATTRIBS_COUNT);
...
int main(int argc, char **argv) {
    ...
    ret = 0;
    printf("main: All tests succeeded.\n");
main_err:
